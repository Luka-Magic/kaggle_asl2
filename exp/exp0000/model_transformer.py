...
torch.set_printoptions(precision=None, threshold=1000000,
...
class Decoder(nn.Module):
    ...
    def __init__(self, feature_size, head_num=16, dropout=0.1):
        ...
        self.mask_multihead = MultiHeadedAttention(
            h=head_num, d_model=feature_size, dropout=dropout)
    ...
    def forward(self, text, global_info, conv_feature, text_length):
        ...
        result = self.mul_layernorm1(
            origin_result + self.mask_multihead(result, result, result, mask=mask))

        b, c, h, w = conv_feature.shape
        conv_feature = conv_feature.view(
        ...
        result = self.mul_layernorm2(
            origin_result + self.multihead(result, conv_feature, conv_feature, mask=None))
...
class Encoder(nn.Module):
    ...
    def __init__(self, output_channel=512, input_channel=256, global_pooling_size=(2, 35), encoder2D=None):
        ...
        self.pe_2D = positionalencoding2d(
            output_channel, global_pooling_size[0], global_pooling_size[1])
    ...
    def forward(self, feature):
        ...
        conv_result = self.relu_bottleneck(
            self.bn_bottleneck(self.cnn_bottleneck(conv_result)))
...
class FeatureEnhancer(nn.Module):
    ...
    def forward(self, conv_feature):
        ...
        result = result
        result = self.mul_layernorm1(
            origin_result + self.mask_multihead(result, result, result, mask=None))
...
class Transformer(nn.Module):
    ...
    def __init__(self, cfg, n_class, feature_size=512):
        ...
        self.pe = PositionalEncoding(d_model=int(
            feature_size / 2), dropout=0.1, max_len=5000)
    ...
    def forward(self, feature, text_length, text_input):
        ...
        global_info = (global_info.squeeze(2).squeeze(2))[
            :, None].repeat(1, text.size(1), 1)
